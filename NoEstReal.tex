...
\begin{demo}
  ...
  \begin{longderivation}<1.1>
      {\Forall{r}[r\in\R^+]{ |h|<r}}\\
    \equiv\\
      {\Forall{r}[r\in\R^+]{ |a\,h| < |a|\,r}}\\
    \why[\To]{$|a|\,r \in \R^+$}\\
      {\Forall{r'}[r'\in\R^+]{|a\,h| < r'}}\\
    \equiv\\
      {a\,h \in M_1}
  \end{longderivation}
  Sea $a\in M_0 - M_1$, por definición, se tiene que:
  \begin{longderivation}<.9>
      {\Exists{r_1,r_2}[r_1,r_2\in\R^+]{r_1 < |a| < r_2}}\\
    \equiv\\
      {\Exists{r_1,r_2}[r_1,r_2\in\R^+]{\frac{1}{r_1} > \frac{1}{|a|} > \frac{1}{r_2}}}\\
    \equiv\\
      {\frac{1}{a}\in M_0 - M_1}
  \end{longderivation}
