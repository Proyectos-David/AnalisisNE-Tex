...
\begin{demo}
  ...
  \begin{longderivation}<.9>
      &{\Forall{n}[n\in\st{\N}]{\Exists{a}[a\in M_0]{|\st{s_n}| \leq a}}}\\
    \why[\To]{Aplicando $\sd{}$}\\
      &{\Forall{n}[n\in\N]{\Exists{a}[a\in M_0]{|s_n| \leq \sd{a}}}}\\
    \equiv\\
      &{\{s\} \text{ es acotada en } \R}
  \end{longderivation}
...
\begin{demo}
  ...
  \begin{longderivation}<1>
      &{\Exists{N}[N\in\N]{\Forall{n}[n\in\N \land n\geq N]{|s_n - s| < \e}}}\\
    \equiv\\
      &{\Exists{N}[N\in\st{\N}]{\Forall{n}[n\in\st{\N} \land n\geq N]{|\st{s_n} - s| < \e}}}\\
    \why{Definición de infinitesimal}\\
      &{\Exists{N}[N\in\st{\N}]{\Forall{n}[n\in\st{\N} \land n\geq N]{\st{s_n} =_1 s}}}\\
    \To\\
...
\begin{longderivation}<.8>
    &{\Forall{\omega}[\omega\in\st{\N}-\N]{\st{s_\omega}=_1 s}}\\
  \equiv\\
    &{\Forall{\omega}[\omega\in\st{\N}-\N]{\sd{\st{s_\omega}}= s}}\\
\end{longderivation}
...
\begin{longderivation}<.8>
  ...
  \equiv\\
    &{n = \sum_{k=0}^n \binom{n}{k} s_n^k}\\
  \To\\
    &{n \geq \binom{n}{2}s_n^2}\\
  \equiv\\
    &{n \geq \frac{n!}{(n-2)!\,2!}s_n^2}\\
  \equiv\\
    &{1 \geq \frac{n-1}{2}s_n^2}\\
  \equiv\\
    &{\sqrt{\frac{2}{n-1}} \geq s_n}
\end{longderivation}
